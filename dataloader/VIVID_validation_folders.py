...
class ValidationSet(data.Dataset):
    ...
    def __init__(
        ...
        if inference_folder == "":
            ...
            self.folders = [
                self.root / folder[:-1] for folder in open(folder_list_path)
            ]
        else:
    ...
    def __getitem__(self, index):
        ...
        print("\033[92m" + str(tgt_thr_img.shape) + "\033[00m")
...
class ValidationSetPose(data.Dataset):
    ...
    def crawl_folders(self, sequence_length=3, step=1):
        ...
        shift_range = np.array(
            [step * i for i in range(-demi_length, demi_length + 1)]
        ).reshape(1, -1)
        for folder in self.folders:
            imgs_thr = sorted(
                (folder / "Thermal").files("*.png")
            )  # "RGB" "Depth" "Thermal" "Thermal_Heat" "Thermal_HeatMM" "Thermal_HeatMMJ"
            poses_T = (
                np.genfromtxt(folder / "poses_T.txt")
                .astype(np.float64)
                .reshape(-1, 3, 4)
            )

            # construct 5-snippet sequences
            tgt_indices = np.arange(demi_length, len(imgs_thr) - demi_length).reshape(
                -1, 1
            )
    ...
    def __getitem__(self, index):
        sample = self.samples[index]
        imgs = [
            np.expand_dims(load_as_float(img), axis=2) for img in sample["thr_imgs"]
        ]
