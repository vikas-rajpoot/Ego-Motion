...
class SequenceFolder(data.Dataset):
    ...
    def __init__(
        ...
        if scene_type == "indoor":
            folder_list_path = (
                self.root / "train_indoor.txt"
                if train
                else self.root / "val_indoor.txt"
            ) 
    ...
    def crawl_folders(self, sequence_length, interval):
        ...
        for folder in self.folders:
            ...
            intrinsics_thr = (
                np.genfromtxt(folder / "cam_T.txt").astype(np.float32).reshape((3, 3))
            ) 
            intrinsics_rgb = (
    ...
    def __getitem__(self, index):
        ...
        ref_trgb_imgs = [
            np.concatenate(
                (
                    np.expand_dims(load_as_float(ref_thr_img), axis=2),
                    load_as_float(ref_rgb_img),
                ),
                axis=2,
            )
